...
## 默认：竖屏 1080 宽、12 环（外沿 657 世界单位）→ zoom = 1080×0.92/1314 ≈ 0.756。
## 环境数（`DEFAULT_CFG.rings_count`）变了这个数就跟着变，它不是常量。
##
## ⚠️ 「换分辨率不改变构图」的前提是拉伸模式为 `canvas_items`（见 `project.godot`）：
##    那时逻辑尺寸不变、所有东西被 canvas 变换统一放大。若用 `viewport` 模式改大视口，
##    世界层会跟着变，但 UI 的写死像素不跟 —— 实测标题字号会缩成一半。
##
